...
import 'package:public_app/Feather/BookTaxi/presentation/widget/buttons/group_button_item_custom.dart';

class GroupButtonBodyCustom<T> extends StatefulWidget {
  const GroupButtonBodyCustom({
    Key? key,
  ...
  @override
  State<GroupButtonBodyCustom<T>> createState() => _GroupButtonBodyState<T>();
}

class _GroupButtonBodyState<T> extends State<GroupButtonBodyCustom<T>> {
  late GroupButtonController _controller;

  @override
  void didUpdateWidget(covariant GroupButtonBodyCustom<T> oldWidget) {
  ...
  List<Widget> _generateButtonsList() {
    ...
    for (var i = 0; i < widget.buttons.length; i++) {
      ...
      if (buttonBuilder != null || buttonIndexedBuilder != null) {
        ...
        button = GroupButtonItemCustom(
          widget: widget.buttons[i] as Widget,
